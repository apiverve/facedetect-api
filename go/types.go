// Package facedetect provides a Go client for the Face Detector API.
//
// For more information, visit: https://apiverve.com/marketplace/facedetect?utm_source=go&utm_medium=readme
package facedetect
...
// Request contains the parameters for the Face Detector API.
//
// Parameters:
//   - url (required): string - URL of the image to analyze (JPG, PNG, GIF, WebP supported) [format: url]
//   - confidence: number - Minimum confidence threshold for face detection (0.1 to 1.0) [min: 0.1, max: 1]
type Request struct {
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		...
		"confidence": {Type: "number", Required: false, Min: float64Ptr(0.1), Max: float64Ptr(1)},
	}
...
// ResponseData contains the data returned by the Face Detector API.
type ResponseData struct {
	Faces []FacesItem `json:"faces"`
	FaceCount int `json:"faceCount"`
	HasFaces bool `json:"hasFaces"`
	ImageWidth int `json:"imageWidth"`
	ImageHeight int `json:"imageHeight"`
}

// FacesItem represents an item in the Faces array.
