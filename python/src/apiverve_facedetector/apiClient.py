...
class FacedetectAPIClientError(Exception):
...
class ValidationError(FacedetectAPIClientError):
...
class FacedetectAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"url": {"type": "string", "required": True, "format": "url"}, "confidence": {"type": "number", "required": False, "min": 0.1, "max": 1, "default": 0.5}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the FacedetectAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises FacedetectAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/facedetect'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise FacedetectAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[FacedetectAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns FaceDetectorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises FacedetectAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise FacedetectAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise FacedetectAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise FacedetectAPIClientError(f"Request failed: {str(e)}")
    ...
    def execute_with_file(self, files: Dict[str, Any], params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log(f"Request exception: {str(e)}")
            raise FacedetectAPIClientError(f"File upload failed: {str(e)}")
    ...
    def execute_with_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not url or not isinstance(url, str):
            raise FacedetectAPIClientError("URL must be provided as a string.")

        if not re.match(r'^https?://.+', url, re.IGNORECASE):
            raise FacedetectAPIClientError("Invalid URL format. URL must start with http:// or https://")
